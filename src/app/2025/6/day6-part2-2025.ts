...
export class Day6Part22025 extends Solution<string, number> {
  ...
  protected override process(input: string): number {
    ...
    const operationAndNumbers: [string, number[]][] = parsedInput[0].map(
      (col, colIndex) => {
        return parsedInput.map(((row: string[]) => row[colIndex]));
      }
    ).map((line) => [line.pop()?.trim() ?? '', this.rearrangeNumberByCol([...line])]);

    console.log(operationAndNumbers);

    const problems: [string, number[]][] = operationAndNumbers
      .map(([operation, numbers]) => {
        const maxNLength = Math.max(...numbers).toString().length;
        const numbersPadded = numbers
          .map(n => n.toString().padEnd(maxNLength, ' '));
        const newNumbers: number[] = [];
        for (let i = 0; i < maxNLength; i++) {
          newNumbers.push(
            parseInt(numbersPadded
              .map((n) => n[i])
              .filter(n => n !== '')
              .join(''))
          );
        }
        return [operation, newNumbers];
      });
